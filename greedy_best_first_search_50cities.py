...
def greedy_route():
    start = CITIES[0]
    goal = CITIES[-1]
    unvisited = CITIES[1:-1][:]
    current = start
    path = [start]
    while unvisited:
        nxt = min(unvisited, key=lambda c: haversine(current, c) * traffic_intensity(12))
        path.append(nxt)
...
if __name__ == "__main__":
    ...
    print_steps(path, "Greedy Best-First Search Algorithm (50 Cities)")
    ...
    save_map(path, "GreedyBestFirst_50cities_route.html")
